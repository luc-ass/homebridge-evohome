...
function SetpointStatus(json) {
   this.targetHeatingTemperature = json.targetHeatingTemperature;
...
Session.prototype.setHeatSetpoint = function (zoneId, targetTemperature, minutes) {
...
Session.prototype._renew = function() {
	...
	return login(credentials.username, credentials.password).then(function(json) {
        self.sessionId = "bearer " + json.access_token;
...
function login(username, password) {
	...
	request({
		...
		body: 'grant_type=password&scope=EMEA-V1-Basic EMEA-V1-Anonymous EMEA-V1-Get-Current-User-Account&Username=' + username + '&Password=' + password
	}, function(err, response) {
		if(err) {
			deferred.reject(err);
		} else {
